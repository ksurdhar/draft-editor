...
const makeRequest = async (
  ...
  // Clean the endpoint for local storage operations
  ...
  if (cleanEndpoint === 'documents/bulk-delete') {
  ...
  const versionMatch = cleanEndpoint.match(/^documents\/([^\/]+)\/versions(?:\?versionId=(.+))?$/)
  if (versionMatch) {
  ...
  const documentMatch = cleanEndpoint.match(/^documents\/([^\/]+)$/)
  if (documentMatch) {
  ...
  const folderMatch = cleanEndpoint.match(/^folders\/([^\/]+)$/)
  if (folderMatch) {
  ...
  if (cleanEndpoint.startsWith('versions/')) {
    console.log('Version operation detected')
    const parts = cleanEndpoint.split('versions/')[1].split('/')
