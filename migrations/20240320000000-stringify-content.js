  ...
  async up(db) {
    ...
    for (const doc of documents) {
      ...
      try {
        ...
        await db
          .collection('documents')
          .updateOne({ _id: doc._id }, { $set: { content: stringifiedContent } })
    ...
    for (const version of versions) {
      ...
      try {
        ...
        await db
          .collection('versions')
          .updateOne({ _id: version._id }, { $set: { content: stringifiedContent } })
  ...
  async down(db) {
    ...
    for (const doc of documents) {
      ...
      try {
        ...
        // Update the document
        await db.collection('documents').updateOne({ _id: doc._id }, { $set: { content: parsedContent } })
    ...
    for (const version of versions) {
      ...
      try {
        ...
        // Update the version
        await db.collection('versions').updateOne({ _id: version._id }, { $set: { content: parsedContent } })
